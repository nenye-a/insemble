...

{
  ...
  if (typeof window !== 'undefined' && typeof window.dispatchEvent === 'function' && typeof document !== 'undefined' && typeof document.createEvent === 'function') {
    ...
    var invokeGuardedCallbackDev = function (name, func, context, a, b, c, d, e, f) {
      ...
      // errors: DELETED_URL_WITH_CREDENTIALS"clearCaughtError was called but no error was captured. This error is likely caused by a bug in React. Please file an issue.");
      }
    }
  }
}
...

{
  warningWithoutStack = function (condition, format) {
    ...
    if (typeof console !== 'undefined') {
      ...
      // breaks IE9: DELETED_URL_WITH_CREDENTIALS"direct" dispatch - there must be at most one dispatch
...
var DELETED_BASE64_STRING = true; // To preserve the "Pause on caught exceptions" behavior of the debugger, we
// replay the begin phase of a failed component inside invokeGuardedCallback.

var DELETED_BASE64_STRING = true; // Warn about deprecated, async-unsafe lifecycles; relates to RFC #6:
...
var enableScopeAPI = false; // New API for JSX transforms to target - DELETED_URL_WITH_CREDENTIALS"prop", "context", "child context"
...
function trackValueOnNode(node) {
  ...
  // DELETED_URL_WITH_CREDENTIALS"Using 'dangerouslySetInnerHTML' in an svg element with " + 'Trusted Types enabled in an Internet Explorer will cause ' + 'the trusted value to be converted to string. Assigning string ' + "to 'innerHTML' will throw an error if Trusted Types are enforced. " + "You can try to wrap your svg element inside a div and use 'dangerouslySetInnerHTML' " + 'on the enclosing div instead.') : void 0;
      }
    }
...
function DELETED_BASE64_STRING(existingQueuedEvent, blockedOn, topLevelType, eventSystemFlags, nativeEvent) {
    ...
    case TOP_FOCUS:
      {
        ...
        queuedFocus = DELETED_BASE64_STRING(queuedFocus, blockedOn, topLevelType, eventSystemFlags, focusEvent);
        return true;
      }
    ...
    case TOP_DRAG_ENTER:
      {
        ...
        queuedDrag = DELETED_BASE64_STRING(queuedDrag, blockedOn, topLevelType, eventSystemFlags, dragEvent);
        return true;
      }
    ...
    case TOP_MOUSE_OVER:
      {
        ...
        queuedMouse = DELETED_BASE64_STRING(queuedMouse, blockedOn, topLevelType, eventSystemFlags, mouseEvent);
        return true;
      }
    ...
    case TOP_POINTER_OVER:
      {
        ...
        queuedPointers.set(pointerId, DELETED_BASE64_STRING(queuedPointers.get(pointerId) || null, blockedOn, topLevelType, eventSystemFlags, pointerEvent));
        return true;
      }
    ...
    case TOP_GOT_POINTER_CAPTURE:
      {
        ...
        queuedPointerCaptures.set(_pointerId2, DELETED_BASE64_STRING(queuedPointerCaptures.get(_pointerId2) || null, blockedOn, topLevelType, eventSystemFlags, _pointerEvent));
        return true;
      }
  }
...
function getEventTarget(nativeEvent) {
  ...
  // DELETED_URL_WITH_CREDENTIALS"entered" or "left" that element.
 */
...
function getPooledWarningPropertyDefinition(propName, getVal) {
  ...
  function warn(action, result) {
    ...
    !warningCondition ? warningWithoutStack$1(false, "This synthetic event is reused for performance reasons. If you're seeing this, " + "you're %s `%s` on a released/nullified synthetic event. %s. " + 'If you must keep the original synthetic event around, use event.persist(). ' + 'See DELETED_URL_WITH_CREDENTIALS"character code" and is safe to use with
 ...
 * @see DELETED_URL_WITH_CREDENTIALS"deltaMode" is reporting in raw wheel delta where one
...
// DELETED_URL_WITH_CREDENTIALS"dispatches", a sequence of listeners and IDs that care about that event.
 *
...
// DELETED_URL_WITH_CREDENTIALS"background-color"
...
function assertValidProps(tag, props) {
  ...
  if (props.dangerouslySetInnerHTML != null) {
    ...
    if (!(typeof props.dangerouslySetInnerHTML === 'object' && HTML$1 in props.dangerouslySetInnerHTML)) {
      {
        throw Error("`props.dangerouslySetInnerHTML` must be in the form `{__html: ...}`. Please visit DELETED_URL_WITH_CREDENTIALS"%s" Client: "%s"', normalizedServerText, normalizedClientText);
  };
  ...
  normalizeHTML = function (parent, html) {
    ...
    // See the discussion in DELETED_URL_WITH_CREDENTIALS"anonymous divs", e.g. the
  // up/down buttons on an <input type="number">. Anonymous divs do not seem to
  // expose properties, triggering a "Permission denied error" if any of its
  // properties are accessed. The only seemingly possible way to avoid erroring
  // is to access a property that typically works for non-anonymous divs and
  // catch any error that may otherwise arise. See
  // DELETED_URL_WITH_CREDENTIALS"Object expected" in some scenarios.
...
function isSameOriginFrame(iframe) {
  try {
    ...
    // DELETED_URL_WITH_CREDENTIALS"in body" mode for
...
function cacheContext(workInProgress, unmaskedContext, maskedContext) {
  if (disableLegacyContext) {
    ...
    instance.__DELETED_BASE64_STRING = unmaskedContext;
...
function getMaskedContext(workInProgress, unmaskedContext) {
  if (disableLegacyContext) {
    ...
    if (instance && instance.__DELETED_BASE64_STRING === unmaskedContext) {
...
// Math.pow(2, 30) - 1
// 0bDELETED_CREDIT_CARD11111111111111
 ...
 * DELETED_URL_WITH_CREDENTIALS"Invalid hook call. Hooks can only be called inside of the body of a function component. This could happen for one of the following reasons:\n1. You might have mismatching versions of React and the renderer (such as React DOM)\n2. You might be breaking the Rules of Hooks\n3. You might have more than one copy of React in the same app\nSee DELETED_URL_WITH_CREDENTIALS"Should not already be working.");
    }
  }
...
function pingSuspendedRoot(root, thenable, suspendedTime) {
  ...
  if (workInProgressRoot === root && renderExpirationTime === suspendedTime) {
    ...
    if (workInProgressRootExitStatus === RootSuspendedWithDelay || workInProgressRootExitStatus === RootSuspended && DELETED_BASE64_STRING === Sync && now() - globalMostRecentFallbackTime < FALLBACK_THROTTLE_MS) {
...
function computeMsUntilSuspenseLoadingDelay(mostRecentEventTime, committedExpirationTime, suspenseConfig) {
  ...
  var eventTimeMs = DELETED_BASE64_STRING(mostRecentEventTime, suspenseConfig);
...
function DELETED_BASE64_STRING(fiber) {
  {
...
function warnIfNotCurrentlyActingEffectsInDEV(fiber) {
  {
    if (warnsIfNotActing === true && (fiber.mode & StrictMode) !== NoMode && IsSomeRendererActing.current === false && IsThisRendererActing.current === false) {
      warningWithoutStack$1(false, 'An update to %s ran an effect, but was not wrapped in act(...).\n\n' + 'When testing, code that causes React state updates should be ' + 'wrapped into act(...):\n\n' + 'act(() => {\n' + '  /* fire events that update state */\n' + '});\n' + '/* assert on the output */\n\n' + "This ensures that you're testing the behavior the user would see " + 'in the browser.' + ' Learn more at DELETED_URL_WITH_CREDENTIALS"Property cannot be accessed on any
