...
Brands:
{
    ...
    match_requests: [                                   -> all the requests for matches related to this brand
        {                                               -> the match schema that is currently active for the brand (perhaps enable different strategies for different regions)
            last_update: ISODate(),
            match_id: ObjectId(),                       -> ID of the matches document in DB_LOCATION_MATCHES
            region_id: ObjectId(),                      -> ID of the region of matches
            type: string,
            status: string,                             -> whether this is from a verified user of the brand
        },
    ]
}
