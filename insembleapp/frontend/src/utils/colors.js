...
// Custom colors specific to Retailer Dashboards, includes the new "grays".
const newColors = {
...
let overrides =
  window.ShardsDashboards && window.ShardsDashboards.colors ? window.ShardsDashboards.colors : {};
  ...
  for (var colorName in overrides) {
    if (overrides.hasOwnProperty(colorName)) {
      if (!/^#([A-Fa-f0-9]{3}$)|([A-Fa-f0-9]{6}$)/.test(overrides[colorName]))
        throw new Error(
          'Please provide a hexadecimal color value if you are trying to override the Retailer Dashboards colors.'
        );
...
export default Object.assign({}, grays, colors, newColors, themeColors, overrides);
