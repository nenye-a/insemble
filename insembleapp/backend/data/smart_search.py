...
searches.create_index(
    [("url",  1), ("params", 1), ("headers", 1)], unique=True)
hosts = ['google', 'yelp', 'foursquare', 'justicemap']
for host in hosts:
    searches[host].create_index(
        [("url", 1), ("params", 1), ("headers", 1)], unique=True)
...
:type URL: string, ex: "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=33.7036519,-118.329670612&type=store&radius=804.67&key=AIzaSyCJjsXi3DbmlB1soI9kHzANRqVkiWj3P2U"
...
:type params: dictionary, ex: params = {client_id=FRSQ_ID, client_secret=FRSQ_SECRET, v='20191028'}
...
def smart_search(URL, host, search_type, params=None, headers=None):
    ...
    host_db_return = searches[host].find_one(
...
def upload_dataset(location, retailer, age, likes, ratings, photo_count):
    ...
    db_return = dataset2.find_one(
        {"name": entry["name"], "lat": entry["lat"], "lng": entry["lng"]})
    if db_return != None:
        print("Entry already exists for name: {0} at {1}, {2}".format(
