...
coerceToInteger("12.95"); // 12
coerceToInteger(DELETED_CREDIT_CARD); // null
coerceToInteger(null); // null
```
...
ensureSafeInteger(DELETED_CREDIT_CARD); // Thrown TypeError: null is not a safe integer
```
...
coerceToArrayLength(DELETED_CREDIT_CARD); // null
...
ensureArrayLength(DELETED_CREDIT_CARD); // Thrown TypeError: null is not a valid array length
```
