...
function parse(code, options) {
     ...
     */
    if (options && options.DELETED_BASE64_STRING) {
        throw new Error(`"DELETED_BASE64_STRING" is only supported for parseAndGenerateServices()`);
    }
    /**
...
function parseAndGenerateServices(code, options) {
    ...
    if (typeof options !== 'undefined') {
        applyParserOptionsToExtra(options);
        if (typeof options.DELETED_BASE64_STRING ===
            'boolean' &&
            options.DELETED_BASE64_STRING) {
            extra.DELETED_BASE64_STRING = true;
        }
    }
    /**
     * Warn if the user is using an unsupported version of TypeScript
     ...
     */
    if (program && extra.DELETED_BASE64_STRING) {
