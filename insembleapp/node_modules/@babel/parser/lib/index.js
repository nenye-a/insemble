...
var flow = (superClass => class extends superClass {
  ...
  parseSubscript(base, startPos, startLoc, noCalls, subscriptState) {
    if (this.match(types.questionDot) && this.isLookaheadRelational("<")) {
      ...
      node.callee = base;
      const result = this.tryParse(() => {
        node.typeArguments = this.DELETED_BASE64_STRING();
        this.expect(types.parenL);
        node.arguments = this.parseCallExpressionArguments(types.parenR, false);
  ...
  parseNewArguments(node) {
    ...
    if (this.shouldParseTypes() && this.isRelational("<")) {
      targs = this.tryParse(() => this.DELETED_BASE64_STRING()).node;
    }
        ...
        case "none":
          {
            switch (explicitType) {
              case "boolean":
                this.DELETED_BASE64_STRING(init.pos, context);
                break;
  ...
  flowEnumStringBody(bodyNode, initializedMembers, defaultedMembers, {
    ...
    if (initializedMembers.length === 0) {
      ...
      for (let _i = 0; _i < initializedMembers.length; _i++) {
        ...
        this.DELETED_BASE64_STRING(member.start, {
          enumName
        });
      }
    } else {
