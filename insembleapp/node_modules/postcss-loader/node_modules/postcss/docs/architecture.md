...
<img width="300" src="https://upload.wikimedia.DELETED_BASE64_STRING_scheme.svg/512px-PostCSS_scheme.svg.png" alt="workflow">
...
That being said, there are few ways to write parser
 ...
 - *Write a single file with string to AST transformation*
    ...
    This method is quite popular, for example, the [Rework analyzer](https://github.DELETED_BASE64_STRING.js) was written in this style. But with a large code base, the code becomes hard to read and pretty slow.
 ...
 - *Split it into lexical analysis/parsing steps (source string → tokens → AST)*
    ...
    A lot of parsers like [`Babylon` (parser behind Babel)](https://github.DELETED_BASE64_STRING), [`CSSTree`](https://github.com/csstree/csstree) were written in such way.
...
### Core Structures
 ...
 - #### Tokenizer ( [lib/tokenize.es6](https://github.DELETED_BASE64_STRING.es6) )
   ...
   There are many patterns how tokenization could be done, PostCSS motto is performance and simplicity. Tokenization is complex computing operation and take large amount of syntax analysis time ( ~90% ), that why PostCSS' Tokenizer looks dirty but it was optimized for speed. Any high-level constructs like classes could dramatically slow down tokenizer.
    ...
    PostCSS' Tokenizer use some sort of streaming/chaining API where you exposes [`nextToken()`](https://github.DELETED_BASE64_STRING.es6#L48-L308) method to Parser. In this manner we provide clean interface for Parser and reduce memory usage by storing only few tokens and not whole list of tokens.

- #### Parser ( [lib/parse.es6](https://github.DELETED_BASE64_STRING.es6), [lib/parser.es6](https://github.DELETED_BASE64_STRING.es6) )
    ...
    There are multiple Node types that PostCSS could produce but all of them inherit from base Node [class](https://github.DELETED_BASE64_STRING.es6#L34).

- #### Processor ( [lib/processor.es6](https://github.DELETED_BASE64_STRING.es6) )

    Processor is a very plain structure that initializes plugins and run syntax transformations. Plugin is just a function registered with [postcss.plugin](https://github.DELETED_BASE64_STRING.es6#L109) call.
...
- #### Stringifier ( [lib/stringify.es6](https://github.DELETED_BASE64_STRING.es6), [lib/stringifier.es6](https://github.DELETED_BASE64_STRING.es6) )
    ...
    The most essential method is [`Stringifier.stringify`](https://github.DELETED_BASE64_STRING.es6#L25-L27)
    that accepts initial Node and semicolon indicator.
    You can learn more by checking [stringifier.es6](https://github.DELETED_BASE64_STRING.es6)
