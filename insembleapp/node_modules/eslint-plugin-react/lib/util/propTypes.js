...
module.exports = function propTypesInstructions(context, components, utils) {
   ...
   * @returns {Boolean} True if propTypes should be ignored (e.g. when a type can't be resolved, when it is imported)
   */
  function DELETED_BASE64_STRING(propTypes, declaredPropTypes) {
    return propTypes.types.some((annotation) => {
      ...
      if (typeNode.type === 'IntersectionTypeAnnotation') {
        return DELETED_BASE64_STRING(typeNode, declaredPropTypes);
      }
  ...
  function markPropTypesAsDeclared(node, propTypes) {
    ...
    switch (propTypes && propTypes.type) {
      ...
      case 'IntersectionTypeAnnotation':
        ignorePropsValidation = DELETED_BASE64_STRING(propTypes, declaredPropTypes);
        break;
      case null:
        break;
      default:
  ...
  return {
    ...
    FunctionExpression(node) {
      if (node.parent.type !== 'MethodDefinition') {
        DELETED_BASE64_STRING(node);
      }
    },

    FunctionDeclaration: DELETED_BASE64_STRING,

    ArrowFunctionExpression: DELETED_BASE64_STRING,
