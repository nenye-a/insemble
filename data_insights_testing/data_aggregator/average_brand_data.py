...
def average_demo_psycho(brand_id):
    '''
    ...
    # average the psychographics
    average_psychographics = average_dictionary_list([location["spatial_psychographics"] for location in locations_list])
    average_demographics = average_dictionary_list([location["environics_demographics"] for location in locations_list])
...
if __name__ == "__main__":
    ...
    data_base_query = {'average_demographics': {}}
    ...
    while True:
        ...
        # brands = utils.DB_BRANDS.find({}, {'_id': 1}).limit(10)

        for brand in brands:
            utils.DB_BRANDS.update({'_id': brand['_id']}, {'$set': average_demo_psycho(brand['_id'])})
