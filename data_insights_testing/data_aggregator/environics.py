...
def get_demographics(lat, lng, radius, demo_df, block_grp_df, cats):
    ...
    ret["Current Year Population 15+, Marital Status"] = dict(zip(cats[39:51], sums[39:51]))
    ret["Current Year Family Households"] = dict(zip(cats[51:57], sums[51:57]))
    ret["Current Year Households, Household Income"] = dict(zip(cats[57:69], sums[57:69]))
    ret["Current Year Median Household Income"] = sums[69]
    ret["Current Year Average Household Income"] = sums[70]
    ret["Current Year Households, Effective Buying Income"] = dict(zip(cats[71:83], sums[71:83]))
    ...
    ret["Current Year Population 25+, Education"] = dict(zip(cats[86:94], sums[86:94]))
    ret["Current Year Workers, Transportation to Work"] = dict(zip(cats[94:100], sums[94:100]))
    ret["Current Year Workers, Travel Time To Work"] = dict(zip(cats[100:103], sums[100:103]))
    ret["Five Year Population, Gender"] = dict(zip(cats[103:105], sums[103:105]))
    ret["Five Year Population, Age"] = dict(zip(cats[105:116], sums[105:116]))
    ret["Five Year Households, Household Income"] = dict(zip(cats[116:128], sums[116:128]))
    ret["Five Year Median Household Income"] = sums[128]
    ret["Five Year Average Household Income"] = sums[129]
    ret["Five Year Households, Effective Buying Income"] = dict(zip(cats[130:142], sums[130:142]))
...
def create_demo_cats_and_df():
    ...
    for line in f[1:]:
        ...
        if line[1] == "":
            continue
        spatial_dict[int(line[0])] = [float(line[i]) for i in range(1, len(line))]
