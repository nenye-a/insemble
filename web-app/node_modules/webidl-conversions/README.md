...
The `long long` and `unsigned long long` Web IDL types can hold values that cannot be stored in JavaScript numbers, so the conversion is imperfect. For example, converting the JavaScript number `DELETED_CREDIT_CARD2000` to a Web IDL `long long` is supposed to produce the Web IDL value `-DELETED_CREDIT_CARD1232`. Since we are representing our Web IDL values in JavaScript, we can't represent `-DELETED_CREDIT_CARD1232`, so we instead the best we could do is `-DELETED_CREDIT_CARD2000` as the output.
