...
[![NPM](DELETED_URL_WITH_CREDENTIALS"clean" and "install" at once
...
For a full example see [node-addon-example's binding.gyp](https://github.com/springmeyer/node-addon-example/blob/DELETED_LONG_HEX_STRING/binding.gyp).
...
For a full example see [node-addon-example's index.js](https://github.com/springmeyer/node-addon-example/blob/DELETED_LONG_HEX_STRING/index.js#L1-L4)
...
For an example of doing this see [node-add-example's .travis.yml](https://github.com/springmeyer/node-addon-example/blob/DELETED_LONG_HEX_STRING/.travis.yml).

Note: if you need 32 bit binaries, this can be done from a 64 bit Travis machine. See [the node-sqlite3 scripts for an example of doing this](https://github.com/mapbox/node-sqlite3/blob/DELETED_LONG_HEX_STRING/scripts/build_against_node.sh#L54-L74).
...
The `binary` properties of `module_path`, `remote_path`, and `package_name` support variable substitution. The strings are evaluated by `node-pre-gyp` depending on your system and any custom build flags you passed.
 ...
 - `node_abi`: The node C++ `ABI` number. This value is available in Javascript as `process.versions.modules` as of [`>= v0.10.4 >= v0.11.7`](https://github.com/joyent/node/commit/DELETED_LONG_HEX_STRING) and in C++ as the `NODE_MODULE_VERSION` define much earlier. For versions of Node before this was available we fallback to the V8 major and minor version.
...
The options are visible in the code at <https://github.com/mapbox/node-pre-gyp/blob/DELETED_LONG_HEX_STRING/lib/util/versioning.js#L114-L127>
